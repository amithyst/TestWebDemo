...
class Command(BaseCommand):
        ...
        for comp in data:
            ...
            obj, created = BooleanComponentType.objects.update_or_create(
                ...
                defaults={
                    ...
                    'true_str': comp.get('yes_string', ''),
                    'false_str': comp.get('no_string', ''),
                    'min_version': min_v,
                    'max_version': max_v,
                }
            )
    ...
    def handle(self, *args, **options):
        ...
        # --- 新增：处理 item_types.json ---
            ...
            # 更新提示信息，加入新的可用文件
            self.stdout.write(self.style.WARNING('Available files: ' \
            'versions.json, materials.json, item_types.json, enchantments.json, ' \
            'attributes.json, effects.json, boolean_components.json'))
